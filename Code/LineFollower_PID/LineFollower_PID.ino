...
LiquidCrystal lcd(13,12,A0,2,3,4);

float speed_right=0.46;
float speed_left=0.6;
const int inh=A5;
const int selA=A4;
const int selB=A3;
const int selC=A2;
...
void maju(unsigned char kiri, unsigned char kanan){
    ...
    pwm_ka=255-pwm_ka;  pwm_ki=255-pwm_ki;
    analogWrite(ena_ka,(pwm_ka*(1-speed_right))); analogWrite(ena_ki,(pwm_ki*(1-speed_left)));
...
void inverse_pid(unsigned char simpan){
  ...
  int outPID = kp*0.01*Error + kd*0.1*(Error - lastError) + ki*0.1;
  lastError = Error;
